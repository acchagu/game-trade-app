...
let platform="",buyPlace="",sellPlace="",bonusPlace="";
...
$("gameForm").onsubmit=e=>{e.preventDefault();if(!platform)return alert("機種を選んでください");if(!buyPlace)return alert("購入場所を選んでください");
let sd=$("sellDate").value, sp=+$("sellPrice").value||0;if((sd||sp)&&!sellPlace)return alert("売却場所を選んでください");
let c=calc(),r={id:Date.now(),title:$("title").value.trim(),platform:val(platform,"platformOther"),buyDate:$("buyDate").value,buyPrice:c.b,buyPlace:val(buyPlace,"buyPlaceOther"),sellDate:sd,sellPrice:c.s,sellTotal:c.s,sellPlace:sellPlace?val(sellPlace,"sellPlaceOther"):"",net:c.n};
let a=recs();a.unshift(r);save(a);e.target.reset();platform=buyPlace=sellPlace="";$("sellPrice").value=0;["platformOther","buyPlaceOther","sellPlaceOther"].forEach(id=>$(id).classList.add("hidden"));build();today();calc();updateSummary();show("list");renderList();renderBonusList()}
function fdate(s){if(!s)return"未売却";let[y,m,d]=s.split("-");return`${y}/${+m}/${+d}`}
function esc(s=""){let d=document.createElement("div");d.textContent=s;return d.innerHTML}
function renderList(){let a=recs().sort((x,y)=>new Date(y.buyDate)-new Date(x.buyDate));$("count").textContent=`${a.length}本 記録中`;$("records").innerHTML=a.length?a.map(r=>`<div class="card"><h3>${esc(r.title)} <span class="badge">${esc(r.platform)}</span></h3><div class="badges"><span class="badge">購入 ${fdate(r.buyDate)}</span><span class="badge">${esc(r.buyPlace)}</span><span class="badge">売却 ${r.sellDate?fdate(r.sellDate):"未売却"}</span>${r.sellPlace?`<span class="badge">${esc(r.sellPlace)}</span>`:""}</div><div class="money"><div><small>購入価格</small><b>${yen(r.buyPrice)}</b></div><div><small>売却総額</small><b>${yen(r.sellTotal)}</b></div><div><small>実質負担額</small><b>${yen(r.net)}</b></div></div>${r.sellDate?`<div class="badges" style="margin-top:8px"><span class="badge">買取 ${yen(r.sellPrice)}</span></div>`:""}<button class="del" onclick="del(${r.id})">削除</button></div>`).join(""):'<div class="empty">まだ記録がありません</div>'}
